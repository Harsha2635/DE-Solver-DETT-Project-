...
lib = ctypes.CDLL("./ForwardEuler.so")

# Define function signature
lib.Forward_euler.argtypes = [
...
lib.Forward_euler.restype = None  # No return value
...
def f_py(t, i, V0, R, L):
    # Square wave: V0 = 5V for 0 ≤ t < 2s, V0 = 0V for 2s ≤ t < 4s, and repeats
    if int(t) % 4 < 2:  
        V_in = V0  # Apply 5V
    else:
...
# Call the C function
lib.Forward_euler(c_f, t0, t_end, i0, h, V0, R, L, t_values, i_values)
...
# Plot the results
plt.plot(t_np, i_np, label="Forward Euler Approximation", linewidth = 1)
plt.xlabel("Time")
plt.ylabel("Current")
